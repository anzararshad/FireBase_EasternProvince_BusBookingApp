...
public class login extends AppCompatActivity {
    ...
    FirebaseAuth.AuthStateListener mAuthlisner; //idu user login aa illiyaa endadda check panura code

    //inda onstart method user login aa illiyaa endadda check panura code
    @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView.setText("SIGHN IN WITH GOOGLE");

        mAuthlisner=new FirebaseAuth.AuthStateListener() {
            @Override
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {
                if(firebaseAuth.getCurrentUser()!=null){
                    startActivity(new Intent(login.this,Main2Activity.class));//source then Destination CLass
                }
    ...
    if (requestCode == RC_SIGN_IN) {
                GoogleSignInResult result = Auth.GoogleSignInApi.getSignInResultFromIntent(data);
                                 if (result.isSuccess()) {
                                     ...
                                     Toast.makeText(getApplicationContext(),"Hey Authentication Went Wrong",Toast.LENGTH_SHORT).show();
                                            }
 }
 }
